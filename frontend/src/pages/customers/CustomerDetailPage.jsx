...
export default function CustomerDetailPage() {
  ...
  const { data: vehiclesData, isLoading: vehiclesLoading } = useQuery({
    queryKey: ['vehicles', { customer_id: id, page: vehiclePage }],
    queryFn: () => getVehicles({ customer_id: id, page: vehiclePage, per_page: 15 }),
  });
  ...
  if (customerError) return <div className="p-6 text-red-500">Failed to load customer.</div>;

  const customer = customerData?.data;
  if (!customer) return <div className="p-6 text-red-500">Customer not found.</div>;

  const vehicles = vehiclesData?.data ?? [];
  const vehicleMeta = vehiclesData?.meta;
    ...
    <div className="p-6 max-w-4xl">
      ...
      <div className="bg-white rounded shadow p-5 mb-6">
        <div className="flex items-start justify-between">
          ...
          <button onClick={() => setEditingCustomer(true)} className="text-sm text-blue-600 hover:underline">
            Edit
          </button>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold text-gray-700">
            Vehicles
            {vehicleMeta && <span className="ml-2 text-sm font-normal text-gray-400">({vehicleMeta.total})</span>}
          </h2>
          <button
        ...
        {vehiclesLoading ? (
          ...
          <>
                  ...
                  {vehicles.map((v) => (
                    <tr key={v.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-right space-x-2">
                        <button onClick={() => setEditingVehicle(v.id)} className="text-blue-600 hover:underline">Edit</button>
                        <button onClick={() => handleDeleteVehicle(v.id, v.plate)} className="text-red-500 hover:underline">Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {vehicleMeta && vehicleMeta.last_page > 1 && (
              <div className="mt-3 flex items-center gap-2">
                <button disabled={vehiclePage === 1} onClick={() => setVehiclePage((p) => p - 1)} className="px-3 py-1 border rounded text-sm disabled:opacity-40">Prev</button>
                <span className="text-sm text-gray-600">Page {vehicleMeta.current_page} of {vehicleMeta.last_page}</span>
                <button disabled={vehiclePage === vehicleMeta.last_page} onClick={() => setVehiclePage((p) => p + 1)} className="px-3 py-1 border rounded text-sm disabled:opacity-40">Next</button>
              </div>
            )}
          </>
        )}
      ...
      {editingCustomer && (
        <CustomerForm customerId={Number(id)} onClose={() => setEditingCustomer(false)} />
      )}
